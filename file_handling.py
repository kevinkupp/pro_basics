...
def merge_dates_and_towns_into_csv(dates_filename: str, towns_filename: str, csv_output_filename: str) -> None:
    ...
    all_names = list(dates.keys()) + [name for name in towns if name not in dates]

    with open(csv_output_filename, "w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["name", "town", "date"])
        for name in all_names:
            writer.writerow([name, towns.get(name, "-"), dates.get(name, "-")])
